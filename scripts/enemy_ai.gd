...
func stun(duration: float = -1.0) -> void:
	stunned_remaining = stunned_duration_seconds if duration < 0.0 else duration
	enemy.player_visible = false
	_set_state(State.STUNNED)


func should_move() -> bool:
	return current_state in [State.PATROLLING, State.CHASING, State.CHASING_LAST_KNOWN_POSITION]
